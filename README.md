...
> 현재 버전: **0.10.0** · Manifest V3 · Chrome 120 이상 · 소스 설치 방식
...
| 작성자 관리 | 한 목록에서 메모와 밴 상태를 함께 관리하고, 갤러리에서도 바로 추가·수정합니다. |
...
4. 갤러리 목록에서는 제목 옆 `참가`와 `+ 기록` 버튼으로 참가하거나 작성자 메모·밴 상태를 바로 관리할 수 있습니다.
...
글쓴이 메모와 밴 목록을 하나의 `작성자 관리` 명단으로 합쳤습니다. 각 항목의 `밴` 체크를 켜면 밴, 끄면 일반 메모가 되며 상태를 바꿔도 작성자 식별 정보와 저장한 메모는 유지됩니다.
...
| 일반 메모 | 파란 정보 표시 | 파란 메모 안내 | 바로 참가 | 유지 |
...
- `작성자 관리` 명단 전체를 접거나 펼칠 수 있습니다.
- 닉네임·식별자·메모 내용으로 검색하고 전체·밴·일반 메모·빈 메모 상태를 필터링할 수 있습니다.
- 평소의 밴 체크와 일괄 선택 체크가 섞이지 않도록 `선택 모드`를 따로 제공합니다.
- 선택한 여러 항목을 한 번에 밴하거나 해제하고, 선택 삭제할 수 있습니다.
- 상태 변경 직후 `실행 취소`로 원래 상태를 복구할 수 있습니다.
- 메모는 항목 안에서 바로 수정하며, 저장 실패 시 편집 중인 문장과 선택 상태를 유지합니다.
- 새 작성자를 추가할 때 `바로 밴`을 선택할 수 있습니다. 일반 메모는 내용이 필요하고 밴 메모는 선택 사항입니다.
- 갤러리 목록의 `+ 기록`·`메모`·`밴` 버튼을 누르면 팝업을 열지 않고도 메모와 밴 여부를 저장하거나 기록을 삭제할 수 있습니다.
- 갤러리 인라인 창은 한 번에 하나만 열리며 `Esc`나 바깥 클릭으로 닫히고, 화면 가장자리에서 잘리지 않도록 위치를 보정합니다.

일반 메모를 새로 추가하거나 저장할 때는 내용이 필요하고, 밴 메모는 선택 사항입니다. 메모 없이 밴을 해제한 기록은 `메모 없음` 필터에서 찾아 보완할 수 있습니다. 메모는 항목당 최대 240자이며 새 작성자 기록은 최대 200개까지 추가할 수 있습니다.
...
| 권한 | 사용 목적 |
| --- | --- |
| `storage` | 설정, unread, 최근 기록, 통합 작성자 명단을 브라우저 로컬 저장소에 보관합니다. |
...
현재 0.10.0 기준 테스트 묶음은 다음을 검증합니다.

- 백그라운드 감지, 저장 직렬화, unread·배지 상태
- 서비스 워커 메시지 라우팅과 선택 권한 수명주기
- DCInside 목록·작성자·마감 제목 파싱
- 콘텐츠 스크립트의 DOM 교체 복구, 참가 버튼과 인라인 작성자 관리 상태
- 일반 메모·밴 경고·숨김·두 단계 참가 확인
- 팝업 통합 명단의 필터, 선택 모드, 일괄 전환, 실행 취소, 실패 시 입력 보존
...
| `scripts/content.js` | 갤러리 목록 관찰과 인라인 참가·작성자 관리 버튼 |
...
- 팝업의 `글쓴이 메모`와 `밴 목록`을 체크 기반 단일 `작성자 관리` 명단으로 통합
- 밴 정책 설정, 작성자 추가, 검색·상태 필터, 개별 편집과 선택 모드를 한 패널에 정리
- 갤러리 목록에 `+ 기록`·`메모`·`밴` 인라인 버튼과 작성자 관리 창 추가
- 겹치는 작성자 규칙을 함께 전환·삭제하고, 넓은 `ㅇㅇ` 닉네임 규칙은 한 번 더 확인
- 일반 메모의 빈 저장 차단, 저장 실패 시 초안 보존, 삭제 재확인과 화면 경계 보정 강화
- 작성자 변경은 팝업과 갤러리 모두 background 단일 writer를 통해 저장
- 구형 분리 목록의 미사용 DOM 참조·상태·문구·CSS를 제거하고 이전 접힘 설정은 새 패널로 안전하게 이전
